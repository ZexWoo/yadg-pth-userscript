      ...
      GM.xmlHttpRequest({
        ...
        onload(response) {
          if (response.status === 200) {
            ...
            const originalImg =
              settingCover === "large"
                ? scaledImg.replace(/_16/, "_0")
                : scaledImg.replace(/_16/, "_10");
...
function LocalStorageWrapper(appPrefix) {
  ...
  const keyPrefix =
    typeof appPrefix === "string" ? appPrefix : JSON.stringify(appPrefix);
...
const yadgSandbox = {
  ...
  initializeSwig(dependencies) {
    ...
    yadgSandbox.exec({
      data: "var myswig = new swig.Swig({ loader: swig.loaders.memory(input.templates), autoescape: false }), i=0; yadg_filters.register_filters(myswig);",
...
factory = {
  ...
  locations: [
    ...
    },
    {
      name: "pth_edit",
      regex:
        /http(s)?:\/\/(.*\.)?redacted\.ch\/torrents\.php\?action=editgroup&groupid=.*/i,
    },
    {
      name: "pth_request",
    ...
    },
    {
      ...
      regex:
        /http(s)?:\/\/(.*\.)?redacted\.ch\/requests\.php\?action=edit&id=.*/i,
    },
    {
    ...
    },
    {
      name: "d3si_edit",
      regex:
        /http(s)?:\/\/(.*\.)?d3si\.net\/torrents\.php\?action=editgroup&groupid=.*/i,
    },
    {
      name: "d3si_request",
    ...
    },
    {
      name: "ops_edit",
      regex:
        /http(s)?:\/\/(.*\.)?orpheus\.network\/torrents\.php\?id=\d+&action=editgroup/i,
    },
    {
      name: "ops_request",
    ...
    },
    {
      ...
      regex:
        /http(s)?:\/\/(.*\.)?orpheus\.network\/requests\.php\?action=edit&id=.*/i,
    },
    {
    ...
    },
    {
      name: "nwcd_edit",
      regex:
        /http(s)?:\/\/(.*\.)?notwhat\.cd\/torrents\.php\?action=editgroup&groupid=.*/i,
    },
    {
      name: "nwcd_request",
    ...
    },
    {
      ...
      regex:
        /http(s)?:\/\/(.*\.)?notwhat\.cd\/requests\.php\?action=edit&id=.*/i,
    },
    {
    ...
    },
    {
      name: "db9_edit",
      regex:
        /https:\/\/www.deepbassnine\.com\/torrents\.php\?action=editgroup&groupid=\d+/i,
    },
    {
      name: "db9_request",
    ...
    },
    {
      ...
      regex:
        /https:\/\/www.deepbassnine\.com\/requests\.php\?action=edit&id=.*/i,
    },
    {
  ...
  init() {
    ...
    if (coverSizeSetting !== null) {
      coverSizeSetting.addEventListener("click", () => {
        ...
        optionsCoverSize.style.display =
          display === "none" || display === "" ? "block" : "none";
      });
    }
    ...
    if (
      lastChecked === null ||
      Date.now() - new Date(lastChecked).getTime() > factory.CACHE_TIMEOUT
    ) {
  ...
  initializeSettings() {
    ...
    if (settingsVer < currentVer) {
      ...
      for (const loc of locations) {
        const replaceDescSettingKey =
          factory.makeReplaceDescriptionSettingsKey(loc);
  ...
  setDescriptionBoxValue(value) {
    ...
    const skipAutoPreview = [
      "pth_torrent_overview",
      "ops_torrent_overview",
      "dic_torrent_overview",
      "db9_torrent_overview",
  ...
  setDefaultTarget() {
    ...
    targetSelect.selectedIndex =
      defaultTarget !== null && defaultTarget in targetOffsets
        ? targetOffsets[defaultTarget]
        : targetOffsets[defaultPTHTarget];
  },
  ...
  setSelect(select, data) {
    ...
    for (const [i, element] of data.entries()) {
      ...
      o.text =
        "nameFormatted" in element ? element.nameFormatted : element.name;
  ...
  getInputElements() {
    ...
    optionsHTML +=
      '<div id="yadg_options_preview_div"><input type="checkbox" name="yadg_options_preview" id="yadg_options_preview" /> <label for="yadg_options_preview" id="yadg_options_preview_label">自动预览描述</label></div>';
    optionsHTML +=
      '<div id="yadg_options_auto_select_scraper_div"><input type="checkbox" name="yadg_options_auto_select_scraper" id="yadg_options_auto_select_scraper"/> <label for="yadg_options_auto_select_scraper" id="yadg_options_auto_select_scraper_label">根据链接自动识别正确的信息源</label></div>';
    optionsHTML +=
      '<div id="yadg_options_links"><a id="yadg_save_settings" class="tooltip" href="#" title="将当前选中的信息源和模板存储为该站的默认选项并保存 API 令牌。">保存设置</a> <span class="yadg_separator">|</span> <a id="yadg_clear_cache" href="#">清除缓存</a></div></div>';
    const inputHTML =
      '<input type="text" name="yadg_input" id="yadg_input" size="60" placeholder="请在此粘贴专辑信息网站的链接"/>';
    ...
    const scraperInfoLink =
      '<a class="tooltip" id="yadg_scraper_info" href="https://yadg.cc/available-scrapers" target="_blank" title="获取更多关于可用信息源的信息">[?]</a>';
    ...
    switch (this.currentLocation) {
        ...
        return (rawData) => {
          ...
          for (const name of Object.keys(inputs)) {
            ...
            if (inputName === "tag_string") {
              ...
              const tagsUnique = tagsArray.filter(
                (element, index, self) => index === self.indexOf(element)
              );
              const tagsFiltered = tagsUnique.filter(
                (element) => element.toLowerCase() !== "electronic"
              );
              const tagsLowercase = tagsFiltered.map((element) =>
                element.toLowerCase()
              );
          ...
          for (const name of Object.keys(artists)) {
            ...
            for (const role of roles) {
              document.querySelector('[name="artists[]"]:last-of-type').value =
                name;
              document.querySelector(
                "#artistfields > #importance:last-of-type"
              ).value = kinds[role];
      ...
      case "pth_request_edit": {
        const f = function (rawData) {
          ...
          if (!/groupid=\d+/.test(document.location.search)) {
            ...
            tagsInput.value =
              data.tags === false ? "" : data.tag_string.toLowerCase();
      ...
      case "waffles_upload": {
        const f = function (rawData) {
          ...
          tagsInput.value =
            data.tags === false ? "" : data.tag_string_nodots.toLowerCase();
...
yadgTemplates = {
  ...
  errorTemplate() {
    yadg.printError("无法获取模板，请另选一个。", true);
  },
};
...
yadg = {
  ...
  authenticationError: "你的 API 令牌无效。请换一个有效的令牌或删除无效的。",
  ...
  makeRequest(parameters) {
    ...
    if (parameters) {
      ...
      data = {
        scraper:
          this.scraperSelect.options[this.scraperSelect.selectedIndex].value,
    ...
    const request = new Requester(resultUrl, "GET", (response) => {
      if (response.status === "done") {
        switch (response.data.type) {
          ...
          case "NotFoundResult": {
            yadg.printError("无法根据给定编号找到任何发行。请换一个试试。");
  ...
  prepareRawResponse(rawData) {
    ...
    if (result.tags !== false) {
      ...
      for (let i = 0; i < result.tags.length; i++) {
        ...
        result.tag_string += result.tags[i]
          .replace(/\s+/g, ".")
          .replace(/\bn\b|&/, "and"); // eslint-disable-line camelcase
    ...
    if (rawData.format) {
      ...
      if (format.includes("album") || rawData.styles.includes("Album")) {
        ...
        format.includes("soundtrack") ||
        rawData.styles.includes("Soundtrack")
      ) {
        result.releaseType = 3;
      } else if (format.includes("ep") || rawData.styles.includes("EP")) {
        result.releaseType = 5;
      } else if (
        format.includes("anthology") ||
        rawData.styles.includes("Anthology")
      ) {
        result.releaseType = 6;
      } else if (
        format.includes("compilation") ||
        rawData.styles.includes("Compilation")
      ) {
        result.releaseType = 7;
      } else if (
        format.includes("single") ||
        rawData.styles.includes("Single")
      ) {
